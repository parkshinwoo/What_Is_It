...
data class StudyRoomDTO(var study_room_id: String?="", // 어린이의 uid
    ...
    data class Message(var message_id: String?="", // 어린이는 uid 뒤에 타임스탬프를 붙인형태, 선생님은 0 뒤에 타임스탬프를 붙인형태
                       ...
                       var message_content: Any?="", // 메세지의 내용. 텍스트, 사진일 경우엔 사진의 uri. 어린의 질문 or 선생님의 답변
                       var question: Any?="", // 어린이의 질문만 담음 for 다이어리
                       var subject: String?="", // 응답 메세지가 어떤 과목 선생님의 답변인지 체크 (과목 체크)
                       var question_id: String?="", // 질문 메세지의 id (응답 메세지와 짝을 이룸)
                       var answer_id: String?="", // 응답 메세지의 id (질문 메세지와 짝을 이룸)
                       var owner_id: String?="" // 메세지 주인의 uid
    )
}